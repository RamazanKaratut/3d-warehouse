...
export default function ResetPasswordPage() {
  const searchParams = useSearchParams(); // Destructuring'i kaldırıyoruz
  ...
  useEffect(() => {
    // URL'den token'ı al
    const urlToken = searchParams.get('token'); // Doğrudan searchParams.get('token') kullanıyoruz
    if (urlToken) {
      setToken(urlToken);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => router.push('/'), 3000); // Başarılı sıfırlamadan sonra giriş sayfasına yönlendir
      } else {
