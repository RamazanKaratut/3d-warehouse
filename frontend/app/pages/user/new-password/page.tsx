...
import { useRouter, useSearchParams } from 'next/navigation'; // useSearchParams'ı import et

export default function NewPasswordPage() {
  const [newPassword, setNewPassword] = useState<string>('');
  const [confirmPassword, setConfirmPassword] = useState<string>('');
  const [message, setMessage] = useState<string>('');
  const [token, setToken] = useState<string | null>(null); // Token'ı tutmak için state
  ...
  useEffect(() => {
    // Sayfa yüklendiğinde URL'den token'ı al
    const urlToken = searchParams.get('token');
    if (urlToken) {
      setToken(urlToken);
      setMessage(''); // Yeni sayfa yüklemede eski mesajı temizle
    } else {
      // Token yoksa kullanıcıyı bilgilendir veya giriş sayfasına yönlendir
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 6) { // Minimum şifre uzunluğu kontrolü
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          router.push('/'); // Giriş sayfasına yönlendir (app/page.tsx)
